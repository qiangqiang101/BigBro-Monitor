...
#End Region
    ...
    Private Sub FormShown(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If _StartPosition = FormStartPosition.CenterParent OrElse _StartPosition = FormStartPosition.CenterScreen Then
            Dim SB As Rectangle = Screen.FromPoint(Cursor.Position).Bounds 'Screen.FromControl(ParentForm).Bounds 'Screen.PrimaryScreen.Bounds
            Dim CB As Rectangle = ParentForm.Bounds
            Dim result As New Point((SB.Width / 2) - (CB.Width / 2), (SB.Height / 2) - (CB.Height / 2))
            If result.X < 0 Then result.X = 0
            If result.Y < 0 Then result.Y = 0
            ParentForm.Location = result
        End If
...
#Region " State Handling "
    ...
    Protected State As eMouseState
    Private Sub SetState(ByVal current As eMouseState)
        State = current
        Invalidate()
    End Sub
    ...
    Protected Overrides Sub OnEnabledChanged(ByVal e As EventArgs)
        If Enabled Then
            SetState(eMouseState.None)
        Else
            SetState(eMouseState.Block)
        End If
        MyBase.OnEnabledChanged(e)
    End Sub

    Protected Overrides Sub OnMouseEnter(ByVal e As EventArgs)
        SetState(eMouseState.Over)
...
Friend Enum eMouseState As Byte
    None = 0
    Over = 1
    Down = 2
    Block = 3
